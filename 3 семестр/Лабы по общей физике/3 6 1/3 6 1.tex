...
Представление периодического сигнала в виде суммы гармонических сигналов называется разложением в ряд Фурье.
	...
	Пусть заданная функция $f(t)$ периодически повторяется с частотой $\Omega_{1}=\dfrac{2\pi}{T},$ где $T$ - период повторения. Её разложение в ряд Фурье имеет вид
\begin{equation}
...
\end{equation}
 ...
 \end{equation}
	...
	Назовём \textit{шириной спектра} $\Delta \omega$ расстояние от главного максимума ($\omega =0$) до первого нуля огибающей, возникающего при $n=\dfrac{2\pi}{\tau \Omega_{1}}$. При этом 
...
\end{equation}
		...
		с основной (несущей) частотой $\omega_{0}$ и амплитудой $a_{\text{осн}} = A_{0}$ . Второе и третье слагаемые соответствуют новым гармоническим колебаниям с частотами $\omega_{0} + \Omega$ и $\omega_{0} - \Omega$. Амплитуды этих двух колебаний одинаковы и составляют $\dfrac{m}{2}$ от амплитуды немодулированного колебания:
		$a_{\text{бок}} = \dfrac{A_{0}m}{2}$. Начальные фазы всех трех колебаний одинаковы.
	\end{enumerate}

\newpage

\section*{Экспериментальная установка}
В работе изучаются спектры периодических электрических сигналов различной формы (последовательности прямоугольных импульсов и цугов, 
а также амплитудно- и фазомодулированных гармонических колебаний). 
Спектры этих сигналов наблюдаются с помощью спектроанализатора, входящего в состав USB-осциллографа и сравниваются с рассчитанными теоретически. Схема установки изображена на рис. 7.
\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{ustanovka.png} % 50% ширины текста
    \caption{Схема экспериментальной установки}
    \label{ustanovka}
\end{figure}

Функциональный генератор позволяет сформировать два различных электрических сигнала, которые выводятся на два независимых канала – CH1 и CH2.
Сигнал с канала CH1 подаётся на вход А, а сигнал с канала CH2 – на вход В USB-осциллографа. Затем эти сигналы подаются на вход компьютера через USB-соединение. При работе USB-осциллографа в режиме осциллографа, на экране компьютера можно наблюдать каждый из сигналов в отдельности, а также их произведение. В режиме спектроанализатора можно наблюдать спектры этих сигналов.
...
\begin{enumerate}
    \item Изначально генератор был настроен на прямоугольные импульсы с частотой повторения $\nu_\text{повт}$ = 1 кГц (период $T$ = 1 мс) и
длительностью импульса $\tau$ = 50 мкс. В результате был получен спектр сигнала (Преобразование Фурье) и исследованы его изменения при разных значениях $\nu_\text{повт}$ и $\tau$.
Из графиков, приведённых ниже, видно, что при увеличении частоты повторения сигнала увеличивается расстояние между компонентами спектра, а при уменьшении длительности сигнала увеличивается ширина спектра.


\begin{figure}[H] % Используйте H вместо h
    \centering
    \includegraphics[width=1\textwidth]{A 1 100.PDF}
    \caption{Спектр при $\nu = 1$ кГц и $\tau = 100$ мкс}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{A 2 100.PDF}
    \caption{Спектр при $\nu = 2$ кГц и $\tau = 100$ мкс}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{A 1 50.PDF}
    \caption{Спектр при $\nu = 1$ кГц и $\tau = 50$ мкс}
\end{figure}



\item Далее были измерены амплитуды $a_n$ и частоты $\nu_n$ спектральных гармоник при фиксированных $\nu_\text{повт}$ и $\tau$ $\left( \nu_n^\text{теор} = \dfrac{n}{T}, \qquad|a_n|_\text{теор} = \dfrac{|\text{sin}\frac{\pi n \tau}{T}|}{\pi n}\right)$.
...
\begin{table}[H]
    ...
    \hline
        ...
        $\nu_{\text{эксп}} $, кГц & 2,01 & 4,026 & 6,042 & 7,983 & 12,01 & 14,03 & 16,05 \\ \hline
        $\nu_{\text{теор}}$, кГц & 2 & 4 & 6 & 8 & 12 & 14 & 16 \\ \hline
        $|a_n|_\text{}$, усл.ед. & 988,3 & 862,4 & 574,6 & 266,6 & 176,7 & 248,7 & 214,9 \\ \hline
        $|a_n/a_1|_\text{}$ & 1 & 0,873 & 0,581 & 0,270 & 0,179 & 0,252 & 0,217 \\ \hline
        $|a_n/a_1|_\text{}$ & 1 & 0,988 & 0,968 & 0,940 & 0,905 & 0,863 & 0,762 \\ \hline
    \end{tabular}
\end{table}



\item Был зафиксирован период повторения прямоугольного сигнала $T = 1$ мс, $\nu_\text{повт} = 1\text{кГц}$. При изменении длительности импульса $\tau$ в диапазоне от 
$\tau=T/50$ до $\tau=T/5$ измерена полная ширина спектра сигнала $\Delta \nu$ — от центра спектра ($\nu = 0$) до гармоники с нулевой амплитудой $a_n \approx 0$.
...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|c|c|c|c|c|c|}
    \hline
        $\tau$, мкс & 20 & 50 & 75 & 100 & 125 & 167 & 200 \\ \hline
        $1/\tau$, 1/мкс & 0,050 & 0,020 & 0,013 & 0,010 & 0,008 & 0,006 & 0,005 \\ \hline
        $\Delta \nu$, кГц & 49,54 & 19,98 & 12,97 & 10,03 & 8,037 & 5,993 & 4,985 \\ \hline
    \end{tabular}
\end{table}

\item Построен график зависимости $\Delta\nu\left(\frac{1}{\tau}\right)$. С помощью МНК определён коэффициент наклона прямой $k = 989\pm9 \approx 1000$ (кГц$\cdot$мкс). Из этого следует, что $\Delta\nu\frac{1}{\tau}=1$, что экспериментально подтверждает соотношение неопределённостей.


\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{А 2.PDF}
    \caption{График зависимости $\Delta\nu\left(\frac{1}{\tau}\right)$}
\end{figure}


\item Теперь была зафиксирована длительность импульса прямоугольного сигнала $\tau = 100 \text{мкс}$. При изменении периода повторения $T$ в диапазоне от $2\tau$ до $50\tau$ были измерены расстояния $\delta\nu = \nu_{n+1} - \nu_n$ между соседними гармониками спектра.

\begin{table}[H]
    \centering
    \begin{tabular}{|c|c|c|c|c|c|c|c|c|c|}
    \hline
        T, мкс & 200 & 500 & 750 & 1000 & 1500 & 2000 & 3000 & 4000 & 5000 \\ \hline
        1/T, 1/мкс & 0,0050 & 0,0020 & 0,0013 & 0,0010 & 0,0007 & 0,0005 & 0,0003 & 0,0003 & 0,0002 \\ \hline
        $\delta \nu$, кГц & 5,013 & 2,000 & 1,271 & 0,996 & 0,693 & 0,500 & 0,333 & 0,250 & 0,200 \\ \hline
    \end{tabular}
\end{table}

\item Построен график зависимости $\delta\nu\left(\frac{1}{T}\right)$. С помощью МНК получен коэффициент наклона прямой $k=1002\pm12\approx1000$ (кГц$\cdot$мкс), то есть $\delta\nu\frac{1}{T}=1$, что также экспериментально подтверждает соотношение неопределённостей.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{А 3.PDF}
...
\begin{enumerate}
	\item На данном этапе выполнения работы генератор изначально был настроен на периодичные импульсы синусоидальной формы (цуги) с несущей частотой $\nu_0$ = 50 кГц, частотой повторения $\nu_\text{повт}$ = 1 кГц, число периодов синусоиды в одном импульсе $N$ = 5 (что соответствует длительности импульса $\tau$ = $N/\nu_o$ = 100 мкс). На осциллографе была получена устойчивая картина сигнала.
Затем изменялись некоторые параметры спектра для наблюдения за его изменением.
	
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Б 50 1 5.PDF}
    \caption{Спектр при $\nu_0 = 50$ кГц, $T = 1$ мс и $N = 5$}
\end{figure}

\item При изменении $\nu_0$ при фиксированных $N$ и $T$ видно, что в этом случае спектр несколько меняет свою форму и сжимается при увеличении частоты.


\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Б 100 1 5.PDF}
    \caption{Спектр при $\nu_0 = 100$ кГц, $T = 1$ мс и $N = 5$}
\end{figure}

\item При изменении $T$ при фиксированных $\nu_0$ и $N$ видно, что форма спектра не меняется, однако он сильно сжимается при увеличении периода повторений.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Б 50 5 5.PDF}
    \caption{Спектр при $\nu_0 = 50$ кГц, $T = 5$ мс и $N = 5$}
\end{figure}


\item При изменении $N$ при фиксированных $\nu_0$ и $T$ видно, что спектр существенно растягивается при увеличении N.


\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Б 50 1 10.PDF}
    \caption{Спектр при $\nu_0 = 50$ кГц, $T = 5$ мс и $N = 10$}
\end{figure}
...
\begin{enumerate}
	\item На генераторе был установлен режим модулированного по амплитуде синусоидального сигнала с  несущей частотой $\nu_0 = 50$ кГц, частотой модуляции $\nu_{\text{мод}} = 2$ кГц, 
глубина модуляции 50$\%$ $(m = 0,5)$. На экране осциллографа получена устойчивая картина сигнала.

\item В режиме курсорных измерений были определены частоты центральной и боковой гармоник при различных значениях несущей частоты. Видно, что при увеличении несущей частоты происходит сдвиг положения спектральных линий влево
\begin{table}[h!]
\centering
% \caption{Результаты измерения частот гармоник при различных несущих частотах}
\begin{tabular}{|c|c|c|}
\hline
$\nu_0$, кГц & Центральная гармоника, кГц & Боковая гармоника, кГц \\
\hline
50 & 1,172 & 0,827 \\
\hline
100 & 0,707 & 0,345 \\
...
\item С помощью осциллографа измерены $A_{max} = 1,295 \text{В}$ и $A_{min} = 449,6 \text{мВ}$, что подтверждает справедливость следующего равенства: $$ m = \frac{A_\text{max} - A_\text{min}}{A_\text{max} + A_\text{min}} = \frac{0,8454}{1,7446} \approx 0,485 \approx 0,5 $$

\item При изменении на генераторе глубины модуляции $m$ в диапазоне от 10 \% до 100 \%, измерены отношение амплитуд боковой и основной 
спектральных линий $a_{\text{бок}}/a_{\text{осн}}$ и построен график зависимости $a_{\text{бок}}/a_{\text{осн}}$ от $m$:

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Гармоники.PDF}
    \caption{Зависимость $a_{\text{бок}}/a_{\text{осн}}$ от $m$}
\end{figure}

\item С помощью МНК получено, что $k=0,4993~m$, что подтверждает равенство $\frac{a_{\text{бок}}}{a_{\text{осн}}}=\frac{m}{2}$, т.е. совпадает с теоретическим предположением.
...
\begin{enumerate}
	\item В цепь была подключена $RC$ цепочка с сопротивлением $R = 3$ кОм и ёмкостью $C = 1000$ пФ. Отсюда характерное время $\tau_{RC} = RC = 3 $ мкс. 
    \item На вход $RC$-цепочки подана последовательность прямоугольных импульсов с периодом повторения $T = 3$ мкс и длительностью $\tau = 150$ нс. На экране получены формы и спектры сигналов при различных периодах повторения сигнала:

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{27 3.PDF}
    \caption{Форма сигнала при $T = 3$ мкс}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{27 3 с.PDF}
    \caption{Спектр при $T = 3$ мкс}
\end{figure}


\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{27 5.PDF}
    \caption{Форма сигнала при $T = 5$ мкс}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{27 5 с.PDF}
    \caption{Спектр при $T = 5$ мкс}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{27 15.PDF}
    \caption{Форма сигнала при $T = 15$ мкс}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{27 15 с.PDF}
    \caption{Спектр при $T = 15$ мкс}
\end{figure}

\item При фиксированной частоте $\nu_0 = 200$ кГц проведены измерения отношений амплитуд соответствующих спектральных гармоник фильтрованного и исходного сигналов: $K_n = |a_n^{\Phi}|/|a_n^0|$:
\begin{table}[h]
\centering
\begin{tabular}{|c|c|c|c|c|c|}
\hline
 n & $a_n^{\Phi}$ & $a_n$ & $\nu_n$, кГц & $a_n^{\Phi}/a_n$ & $1/\nu_n$, мс\\ \hline
        1 & 65 & 283 & 200 & 0,2297 & 0,0050 \\ \hline
        2 & 30 & 267 & 400 & 0,1124 & 0,0025 \\ \hline
        3 & 19 & 248 & 600 & 0,0766 & 0,0017 \\ \hline
        4 & 16 & 252 & 800 & 0,0635 & 0,0013 \\ \hline
        5 & 14 & 268 & 1000 & 0,0522 & 0,0010 \\ \hline
        6 & 9 & 250 & 1200 & 0,0360 & 0,0008 \\ \hline
        7 & 6 & 242 & 1400 & 0,0248 & 0,0007 \\ \hline
\end{tabular}
% \caption{Таблица значений}
% \label{tab:my_label}
\end{table}
\item Построены графики зависимости $K(\nu)$ и $K(1/\nu)$.
\item Зависимость $K(1/\nu)$ линейная:
\begin{equation}
    K(1/\nu) = \frac{1}{2\pi \tau_{RC}} \left(\frac{1}{\nu}\right)
\end{equation}
Таким образом, из коэффициента наклона прямой $\kappa = 45,98 $ кГц можно определить $\tau_{RC} = 1/\kappa(2\pi) \approx 3,46 $ мкс, что близко к рассчитанному изначально $\tau_{RC} = 3$ мкс.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{К.PDF}
    \caption{График зависимости $K(\nu)$}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{К л.PDF}
...
\newpage
\section*{Выводы}
В ходе выполнения лабораторной работы были экспериментально исследованы основы спектрального анализа и преобразований Фурье. На практике изучены спектральные характеристики различных периодических электрических сигналов: прямоугольных импульсов, цугов синусоидальных колебаний, амплитудно-модулированных сигналов, а также фильтрованных сигналов.

С помощью осциллографа были получены спектры сигналов, позволившие определить их частотный состав. Экспериментально подтверждено соотношение неопределённостей $\Delta \nu \cdot \Delta t \simeq 1$, связывающее временные и частотные характеристики сигнала.

% В процессе работы освоены методы обработки экспериментальных данных, включая построение графиков зависимостей и их аппроксимацию методом наименьших квадратов. Полученные результаты демонстрируют практическую значимость спектрального анализа для исследования электрических сигналов и подтверждают теоретические положения о свойствах преобразования Фурье.
...
%     \caption{Периодическая последовательность гауссианов и её спектр}
%     \label{gausiana}
% \end{figure}
